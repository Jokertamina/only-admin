  ...
  try {
    ...
    if (plan === "BASICO") {
      try {
        ...
        console.error("[stripe-create] Error durante el downgrade:", error);

        // Si ocurre un error antes de devolver una respuesta al cliente, intentamos revertir el estado `downgradePending`
        await empresaRef.update({
          downgradePending: false,
        });

        // Propagamos un error controlado al cliente
        return NextResponse.json({
          error: "Ocurrió un problema durante el downgrade. Inténtalo de nuevo más tarde.",
        }, { status: 500 });
      }
    }
